...
static void monitor_soc_work(struct work_struct *work)
{
	...
	power_supply_changed(&chip->bms_psy);
	printk("battery temp is %d,voltage is %d,calculated soc is %d,battery status is %d\n",batt_temp,vbat_uv/1000,chip->calculated_soc,get_battery_status(chip));
	mutex_unlock(&chip->last_soc_mutex);
...
static int calculate_initial_soc(struct qpnp_bms_chip *chip)
{
	...
	if (chip->warm_reset) {
		...
		if (!shutdown_soc_invalid &&
			...
			pr_info("Using shutdown SOC, ocv calculated soc is %d\n",chip->calculated_soc);
			chip->last_ocv_uv = chip->shutdown_ocv;
			chip->last_soc = chip->shutdown_soc;
			chip->calculated_soc = lookup_soc_ocv(chip,
						chip->shutdown_ocv, batt_temp);
...
#ifdef CONFIG_MULTI_BATTERY
	chip->bat_id = read_battery_id(chip);
	pr_err("get bat_id %lld 0x%x\n", chip->bat_id, (unsigned int)chip);		
#endif
//lenovo-sw weiweij add second battery supplier support	end
...
static int qpnp_vm_bms_probe(struct spmi_device *spmi)
{
	...
	chip->batt_data = &lenovo_2500_data;
