	...
	*power_setting_size = count;
	...
	for (i = 0; i < count; i++) {
		...
		case SENSOR_GPIO:
			...
			else if (!strcmp(seq_name, "sensor_gpio_reset_ex"))
				ps[i].seq_val = SENSOR_GPIO_RESET_EX;
			...
			else if (!strcmp(seq_name, "sensor_gpio_vdig_ex"))
				ps[i].seq_val = SENSOR_GPIO_VDIG_EX;
/*+ ljk for other gpio as power supply*/				
	...
	uint16_t gpio_array_size)
{
	...
	if (rc != -EINVAL) {
		...
		rc = 0;
	if (of_property_read_bool(of_node, "qcom,gpio-vdig-ex") == true) {
		rc = of_property_read_u32(of_node, "qcom,gpio-vdig-ex", &val);
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-vdig-ex failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-vdig-ex invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_VDIG_EX] =
			gpio_array[val];
		gconf->gpio_num_info->valid[SENSOR_GPIO_VDIG_EX] = 1;
		CDBG("%s qcom,gpio-vdig-ex %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_VDIG_EX]);
	}

	/*+Begin: add ljk for gpio set.*/
	...
	if (rc != -EINVAL) {
		...
		rc = 0;
	if (of_property_read_bool(of_node, "qcom,gpio-reset-ex") == true) {
		rc = of_property_read_u32(of_node, "qcom,gpio-reset-ex", &val);
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-reset-ex failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_err("%s:%d qcom,gpio-reset-ex invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_RESET_EX] =
			gpio_array[val];
		gconf->gpio_num_info->valid[SENSOR_GPIO_RESET_EX] = 1;
		CDBG("%s qcom,gpio-reset-ex %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_RESET_EX]);
	}
