...
extern struct delayed_work lcd_color_work;
extern struct mutex color_lock;
...
#ifdef ESD_FOR_LCD
static char lcd_color[2] = {0x38, 0x0};	/* DTYPE_DCS_WRITE1 */
static struct dsi_cmd_desc color_cmd = {
	{DTYPE_DCS_WRITE1, 1, 0, 0, 1, sizeof(lcd_color)},
	lcd_color
};
void mdss_dsi_panel_color_dcs(struct mdss_dsi_ctrl_pdata *ctrl)
{
	struct dcs_cmd_req cmdreq;
...
static int mdss_dsi_request_gpios(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (gpio_is_valid(ctrl_pdata->disp_vsp_gpio)) {
		...
		if (rc) {
			pr_err("request disp vsp gpio failed, rc=%d\n",
					rc);
	...
	if (gpio_is_valid(ctrl_pdata->disp_vsn_gpio)) {
		...
		if (rc) {
			pr_err("request disp vsn gpio failed, rc=%d\n",
					rc);
...
static int mdss_dsi_request_gpios(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	rc = gpio_request(ctrl_pdata->lcd_enp_gpio, "lcd_enp");
        if (rc) {
                pr_err("request lcd enp gpio failed,[%d]\n",ctrl_pdata->lcd_enp_gpio);
		goto failed;
        }
	rc = gpio_request(ctrl_pdata->lcd_enn_gpio, "lcd_enn");
        if (rc) {
                pr_err("request lcd enn gpio failed,[%d]\n",ctrl_pdata->lcd_enn_gpio);
		goto failed;
        }
	return rc;
...
failed:
	if (gpio_is_valid(ctrl_pdata->lcd_enn_gpio))
                gpio_free(ctrl_pdata->lcd_enn_gpio);
	if (gpio_is_valid(ctrl_pdata->lcd_enp_gpio))
                gpio_free(ctrl_pdata->lcd_enp_gpio);
	if (gpio_is_valid(ctrl_pdata->rst_gpio))
                gpio_free(ctrl_pdata->rst_gpio);
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
...
#ifndef CONFIG_PRODUCT_AIRPLAY
	if (!gpio_is_valid(ctrl_pdata->disp_en_gpio)) {
		pr_err("%s:%d, disp_en line not configured\n",
			   __func__, __LINE__);
	}
#endif
	if (!gpio_is_valid(ctrl_pdata->rst_gpio)) {
	...
	if (enable) {
...
#ifdef CONFIG_PRODUCT_AIRPLAY
		ret = gpio_direction_output(ctrl_pdata->lcd_enp_gpio, 1);
                if (ret)
                        pr_err("Unable to set output for ENP");
		mdelay(1);
		ret = gpio_direction_output(ctrl_pdata->lcd_enn_gpio, 1);
                if (ret)
                        pr_err("Unable to set output for ENN");
#endif
		...
		if (!pinfo->panel_power_on) {
...
#endif
			/* lenovo.sw2 houdz1 20140409:add for lcd VSP/VSN enable start*/
			if(of_board_is_z2())
			{
				if (gpio_is_valid(ctrl_pdata->rst_gpio))
			...
			for (i = 0; i < pdata->panel_info.rst_seq_len; ++i) {
				...
				gpio_direction_output((ctrl_pdata->rst_gpio),pdata->panel_info.rst_seq[i]);//lenovo.sw2 houdz1 modify 20140409
			...
			if(of_board_is_z2())
			{
				if(ctrl_pdata->panel_id == 1)//tianma
				{
					if (gpio_is_valid(ctrl_pdata->disp_vsp_gpio))
						//gpio_set_value((ctrl_pdata->disp_vsp_gpio), 1);
						gpio_direction_output((ctrl_pdata->disp_vsp_gpio), 1);
...
#ifdef CONFIG_PRODUCT_AIRPLAY
		pr_err("lcd enp gpio [%d]\n",ctrl_pdata->lcd_enp_gpio);
		pr_err("lcd enn gpio [%d]\n",ctrl_pdata->lcd_enn_gpio);

		if (gpio_is_valid(ctrl_pdata->lcd_enp_gpio)) {
			gpio_set_value((ctrl_pdata->lcd_enp_gpio), 0);
		}
		if (gpio_is_valid(ctrl_pdata->lcd_enn_gpio)) {
			gpio_set_value((ctrl_pdata->lcd_enn_gpio), 0);
		}

#endif
		if(of_board_is_sisleyl()) //lenovo.sw2 add for sisleyl (boe)
		{
			if (gpio_is_valid(ctrl_pdata->rst_gpio))gpio_set_value((ctrl_pdata->rst_gpio), 0);
			mdelay(2);
			if (gpio_is_valid(ctrl_pdata->disp_vsp_gpio)) gpio_set_value((ctrl_pdata->disp_vsp_gpio), 0);
			if (gpio_is_valid(ctrl_pdata->disp_vsn_gpio)) gpio_set_value((ctrl_pdata->disp_vsn_gpio), 0);
			mdelay(5);
		}
		else
		{
	...
	if(panel_z2_tianma_is_esd_recover(ctrl_pdata)) mdelay(100);
	}
/*lenovo.sw2 houdz1 add (end): add "disp_vsp_gpio" and "disp_vsp_gpio"*/
...
void enable_te(struct mdss_dsi_ctrl_pdata *ctrl, int enable)
{
	...
	else if(of_board_is_z2() || of_board_is_sisley())  //lwnovo.sw2 houdz1 add for project z2
	{
		if (enable) {
			if (!te_running && gpio_is_valid(ctrl->disp_te_gpio)) {
				te_running = 1;
 ...
 #ifdef CONFIG_LENOVO_LCD_EFFECT//lenovo.sw2 houdz1 add for lcd effect
...
int g_ce_level=2;//lenovo.sw2 houdz1 add for sisleyl:ce mode
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	if(of_board_is_sisleyl()) g_ce_level = 1;
#ifdef CONFIG_LENOVO_LCD_EFFECT//lenovo.sw2 houdz1 add for lcd effect
	...
	enable_te(ctrl, 1);
	if (ctrl->lcd_id) {
		printk("============ enable color work ==============\n");
#ifdef ESD_FOR_LCD
		color_enable = 1;
		schedule_delayed_work(&lcd_color_work, 0);
#endif	
	}
	pr_info("%s () done\n", __func__);
	pr_debug("%s:-\n", __func__);
...
static int mdss_dsi_panel_off(struct mdss_panel_data *pdata)
{
	...
	if (ctrl->lcd_id) {
		...
		color_enable = 0;
		cancel_delayed_work(&lcd_color_work);
		printk("============ disable color work ==============\n");
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->ce_on_cmds,
		"lenovo,lcd-effect-ce-on-command", "lenovo,lcd-effect-ce-on-command-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->ce_off_cmds,
		"lenovo,lcd-effect-ce-off-command", "lenovo,lcd-effect-ce-off-command-state");
...
/*lenovo.sw2 houdz1 add  (sisley-lite) -- start*/
...
#endif
static void mdss_dsi_panel_set_ce_level(struct mdss_panel_data *pdata,u32 level)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	struct dcs_cmd_req cmdreq;
	struct dsi_panel_cmds *pCmdDesc = NULL ;
	...
	memset(&cmdreq, 0, sizeof(cmdreq));
	if(level == 0)  pCmdDesc = &ctrl_pdata->ce_off_cmds;
	else pCmdDesc = &ctrl_pdata->ce_on_cmds;
	if(pCmdDesc->cmd_cnt) {
		cmdreq.cmds = pCmdDesc->cmds;
		cmdreq.cmds_cnt = pCmdDesc->cmd_cnt;
		//lenovo_show_lcd_param(cmdreq.cmds,cmdreq.cmds_cnt);
	}else{
		pr_err("[houdz1]%s:cmds is not exit!!level=%d\n",__func__,level);
		return ;
	}
	cmdreq.flags = CMD_REQ_COMMIT;
	cmdreq.rlen = 0;
	cmdreq.cb = NULL;
	if (pCmdDesc->link_state == DSI_LP_MODE) cmdreq.flags  |= CMD_REQ_LP_MODE;
	...
	bool cmd_cfg_cont_splash)
{
	...
	if(of_board_is_sisleyl()){//lenovo.sw2 houdz1 add (sisley-lite)
		...
		ctrl_pdata->panel_data.set_ce_level = mdss_dsi_panel_set_ce_level;
	}
