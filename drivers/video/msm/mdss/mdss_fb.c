...
#ifndef CONFIG_LENOVO_LCD_EFFECT
extern int g_ce_level;
static ssize_t mdss_panel_get_ce(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	/*lenovo.sw2 houdz1 add for sisleyl ce(start)*/
	if(of_board_is_sisleyl()) {
		ret = snprintf(buf, PAGE_SIZE, "%d\n",g_ce_level);
		return ret;
	}
	/*lenovo.sw2 houdz1 add for sisleyl ce(end)*/
			 ...
			 const char *buf, size_t count)
{
	...
	/*lenovo.sw2 houdz1 add for sisleyl ce(start)*/
	if(of_board_is_sisleyl()) {
		if(level == 0) g_ce_level = 0;
		else g_ce_level = 1;
		pdata = dev_get_platdata(&mfd->pdev->dev);
		if ((pdata) && (pdata->set_ce_level)){
			pdata->set_ce_level(pdata, level);
...
static int g_bl_level=0;//lenovo.sw2 houdz1 add for sisleyl:ultra mode
			 ...
			 const char *buf, size_t count)
{
	...
	if(of_board_is_sisleyl())
	{
		if(!mfd->panel_power_on) return 0;
		pdata = dev_get_platdata(&mfd->pdev->dev);
	...
	else ret = handle_lcd_effect_data(mfd, &lcd_data, &ctrl);
...
static DEVICE_ATTR(ce_onoff, S_IRUGO | S_IWUSR | S_IWGRP, lenovo_fb_get_ce, lenovo_fb_set_ce);
static DEVICE_ATTR(cabc_onoff, S_IRUGO | S_IWUSR | S_IWGRP, lenovo_fb_get_cabc, lenovo_fb_set_cabc);
static DEVICE_ATTR(dimming_onoff, S_IRUGO | S_IWUSR | S_IWGRP, lenovo_fb_get_dimming, lenovo_fb_set_dimming);
static DEVICE_ATTR(lcd_effect, S_IRUGO | S_IWUSR | S_IWGRP, lenovo_fb_get_effect, lenovo_fb_set_effect);
static DEVICE_ATTR(lcd_mode, S_IRUGO | S_IWUSR | S_IWGRP, lenovo_fb_get_mode, lenovo_fb_set_mode);
static DEVICE_ATTR(lcd_supported_effect, S_IRUGO | S_IWUSR | S_IWGRP, lenovo_fb_get_lcd_supported_effect, NULL);
static DEVICE_ATTR(lcd_supported_mode, S_IRUGO | S_IWUSR | S_IWGRP, lenovo_fb_get_lcd_supported_mode, NULL);
static DEVICE_ATTR(lcd_effect_index, S_IRUGO | S_IWUSR | S_IWGRP, lenovo_fb_get_effect_index, lenovo_fb_set_effect_index);
//static DEVICE_ATTR(lcd_effect_debug_onoff, S_IRUGO | S_IWUSR | S_IWGRP, lenovo_fb_get_lcd_effect_debug_onoff, lenovo_fb_set_lcd_effect_debug_onoff);
static DEVICE_ATTR(bl_level, S_IRUGO | S_IWUSR | S_IWGRP, lenovo_fb_get_bl_gpio_level, lenovo_fb_set_bl_gpio_level);
...
#ifdef CONFIG_LENOVO_LCD_EFFECT	
	&dev_attr_lcd_effect_index.attr,
	//&dev_attr_lcd_effect_debug_onoff.attr,
#endif
	NULL,
};
...
static int mdss_fb_probe(struct platform_device *pdev)
{
	...
	mfd->pdev = pdev;
#ifdef ESD_FOR_LCD  //lenovo.sw2 houdz1 add for lcd esd check
	pr_debug("[houdz]mfd->index =%d.\n",mfd->index);
	if (mfd->index == 1) 	mfd_priv = mfd;
#endif
	if (pdata->next)
...
#ifdef CONFIG_LENOVO_LCD_EFFECT
	ret = lenovo_fb_lcd_effect_handle(mfd, &data);
#else
			 ...
			 unsigned long arg)
{
	...
	case MSMFB_PANEL_EFFECT:
		if(of_board_is_z2()) ret = mdss_fb_panel_effect(mfd, argp);
		else goto exit;
		break;
