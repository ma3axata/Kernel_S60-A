...
/* lenovo-sw, lily8, 2014-09-11,for micbias-ext-cap-mode from QC patch,  begin*/
...
/* lenovo-sw, lily8, 2014-09-11,for micbias-ext-cap-mode from QC patch,  end*/
...
/*add by lily8@lenovo.com for autotestA start 2014-07-15*/
		...
		pr_debug("%s: headset_headphone_flag(%d)\n",
		 __func__,headset_headphone_flag);
		#endif
/*add by lily8@lenovo.com for autotestA end 2014-07-15*/

	} else if (plug_type == MBHC_PLUG_TYPE_GND_MIC_SWAP) {
		/*lenovo-sw,lily8,2014-12-19,for slowly plug in,begin*/
		if (mbhc->current_plug == MBHC_PLUG_TYPE_HEADPHONE)
			wcd_mbhc_report_plug(mbhc, 0,
						SND_JACK_HEADPHONE);
		if (mbhc->current_plug == MBHC_PLUG_TYPE_HEADSET)
			wcd_mbhc_report_plug(mbhc, 0, SND_JACK_HEADSET);
		/*lenovo-sw,lily8,2014-12-19,for slowly plug in,end*/
...
static bool wcd_is_special_headset(struct wcd_mbhc *mbhc)
{
	...
	while (result2 & 0x01)  {
		...
		snd_soc_update_bits(codec, MSM8X16_WCD_A_ANALOG_MICB_1_CTL,
				    0x60, 0x60);
		/*lenovo_sw lily8,2014-9-08,for apple headset detect,micbias 2.85v, begin*/
		snd_soc_write(codec, MSM8X16_WCD_A_ANALOG_MICB_1_VAL,
			      0xC8);
		/*lenovo_sw lily8,2014-9-08,for apple headset detect,micbias 2.85v, end*/
...
exit:
	...
	if (plug_type == MBHC_PLUG_TYPE_HEADPHONE)
	{
		pr_debug("%s: report headphone and schedule start",__func__);
		wcd_mbhc_find_plug_and_report(mbhc, plug_type);
...
static irqreturn_t wcd_mbhc_release_handler(int irq, void *data)
{
	 ...
	 */
	//zhougd2:if current type is not headset but release interrupt happen, detect_plug_type again
	//fix bug of hook report when insert by half
	//wait for better resolution from qualcomm

	if (mbhc->current_plug == MBHC_PLUG_TYPE_HEADPHONE) {
		if (!snd_soc_read(mbhc->codec, MSM8X16_WCD_A_ANALOG_MBHC_BTN_RESULT) && !snd_soc_read(mbhc->codec,
					MSM8X16_WCD_A_ANALOG_MBHC_ZDET_ELECT_RESULT))
			wcd_mbhc_report_plug(mbhc, 1, SND_JACK_HEADSET);
		goto exit;
	}
		      ...
		      bool impedance_det_en)
{
	...
	mbhc->micbias2_cap_mode =
		...
		MICBIAS_EXT_BYP_CAP : MICBIAS_NO_EXT_BYP_CAP);
	pr_debug("\n %s: micbias1_cap_mode= %d micbias2_cap_mode = %d\n", __func__, mbhc->micbias1_cap_mode,
			mbhc->micbias2_cap_mode);
	/* lenovo-sw, lily8, 2014-09-11,for micbias-ext-cap-mode from QC patch,  end*/
