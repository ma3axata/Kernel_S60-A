...
extern void msm8x16_enable_ext_spk_power_amp(u32 on);
static int msm8x16_wcd_put_ext_pa_enable(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value *ucontrol);
static int msm8x16_wcd_get_ext_pa_enable(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value *ucontrol);
static int flag_ext_pa_on = 0;

static int msm8x16_wcd_put_ext_pa_enable(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value *ucontrol)
{
	int value = ucontrol->value.integer.value[0];
	pr_info("%s:value %d\n",__func__,value);
	flag_ext_pa_on = value ? 1 : 0;
	msm8x16_enable_ext_spk_power_amp(value);
	return 0;
}
static int msm8x16_wcd_get_ext_pa_enable(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = flag_ext_pa_on;
	pr_info("%s:value %d\n",__func__,flag_ext_pa_on);
	return 0;
}
#endif
...
static const struct snd_kcontrol_new msm8x16_wcd_snd_controls[] = {
	...
	msm8x16_wcd_put_iir_enable_audio_mixer),
#ifdef CONFIG_PRODUCT_AIRPLAY
	SOC_SINGLE_EXT("EXT_PA_SWITCH",-1,0,1,0,
	msm8x16_wcd_get_ext_pa_enable,
	msm8x16_wcd_put_ext_pa_enable),
#endif
				     ...
				     struct snd_kcontrol *kcontrol, int event)
{
	...
	dev_dbg(w->codec->dev, ">>>>%s %d %s, boost_set:%d\n", __func__, event, w->name, msm8x16_wcd->spk_boost_set);
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_update_bits(codec,
...
static const struct msm8x16_wcd_reg_mask_val
	 ...
	 */
	{MSM8X16_WCD_A_ANALOG_RX_COM_OCP_CTL, 0xFF, 0xDF}, //lenovo-sw zhouwl,for headset esd,qcom original 0xD1
