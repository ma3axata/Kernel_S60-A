...
static long lowmem_threshold_size = (50*1024*1024 + 16*1024*1024);    //50M low memory threshod, 16M for reserve
...
static int g_nSystemAppPackageCount = 11;


bool isCanWrite(struct file *file, size_t len)
{
    ...
    if(memcmp(mount_data->mnt_mountpoint->d_name.name, "data", 4) == 0)
    {
        ...
        if ((vfs_statfs(&(file->f_path), &sbuf)) == 0)
        {
            ...
            /*lenovo-sw jixj 2014.6.2 modified begin*/
            #if 0
            if(freemem < lowmem_threshold_size)
            #else
            if(freemem < (lowmem_threshold_size + len))
            #endif
            /*lenovo-sw jixj 2014.6.2 modified end*/
            {
                bIsLowMem = true;
                printk(" /data low freemem detected freemem=%llu\n",freemem);
            }
        }
		...
		unsigned long nr_segs, loff_t pos)
{
    ...
    if(!isCanWrite(iocb->ki_filp, length))
    {
        return -ENOSPC;
    }
...
static int __init lowmemdetect_init(void)
{
    ...
    if (!IS_ERR(myfile) && (myfile->f_op != NULL))
    {
        ...
        lenovo_file_operations = (struct file_operations *)myfile->f_op;    //it point to ext4_file_operations
